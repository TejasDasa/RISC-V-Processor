module dmem #(
    ...
    assign addr_valid = (addr >= DMEM_BASE) && (addr < DMEM_BASE + DEPTH*4);
    ...
    always_ff @(posedge clk) begin
        ...
        if (mem_write_en && addr_valid) begin
            if (byte_en[0])
                mem[local_addr[9:2]][7:0] <= write_data[7:0];

            if (byte_en[1])
                mem[local_addr[9:2]][15:8] <= write_data[15:8];

            if (byte_en[2])
                mem[local_addr[9:2]][23:16] <= write_data[23:16];

            if (byte_en[3])
                mem[local_addr[9:2]][31:24] <= write_data[31:24];
        end
    end
